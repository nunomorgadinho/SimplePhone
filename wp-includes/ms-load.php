...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'Esse site não existe. Por favor tente <a href="%s">%s</a>.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'Nenhum site definido neste servidor. Se é o dono deste site, por favor, verifique <a href="http://codex.wordpress.org/Debugging_a_WordPress_Network">Debugging de uma Rede WordPress</a> para obter ajuda.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'Erro ao estabelecer ligação à base de dados'/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'Se seu site não for exibido, entre em contato com o proprietário desta rede.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'Se é o proprietário da rede verifique que o MySQL está a funcionar correctamente e que todas as tabelas estão livres de erros.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>Tabelas da base de dados em falta.</strong> Isto significa que o MySql não está a correr, o Wordpress não foi instalado correctamente ou alguém eliminou <code>%s</code>. Deverá agora mesmo verificar a sua base de dados.'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>Não foi Possível Localizar o site <code>%1$s</code></strong> Procurada a tabela<em>%2$s</em> na base de dados <code>%3$s</code>. Está correcto?'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'What do I do now?'/*WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'Leia a página sobre a <a target="_blank" href="http://codex.wordpress.org/Debugging_a_WordPress_Network">relação de bugs</a>. Algumas das orientações nessa página podem ajudá-lo a descobrir o que deu errado.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'Se continua a ver esta mensagem, por favor verifique se a sua base de dados contém as seguintes tabelas:'/*/WP_I18N_STUCK*/ . '</p><ul>';
