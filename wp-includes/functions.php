	...
	foreach ( $wp_tables as $table ) {
		...
		if ( in_array( $table, $tables ) ) {
			...
			if ( defined('WP_REPAIRING') )
				return true;
			// Die with a DB error.
			$wpdb->error = sprintf( /*WP_I18N_NO_TABLES*/'Uma ou mais tabelas da base de dados não estão disponíveis. A base de dados pode estar a precisar de ser <a href="%s">reparada</a>.'/*/WP_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed' );
			dead_db();
		}
	}
