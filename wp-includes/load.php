...
function wp_unregister_GLOBALS() {
	...
	if ( isset( $_REQUEST['GLOBALS'] ) )
		die( /*WP_I18N_GLOBALS_OVERWRITE*/'Detectada tentativa de rescrever GLOBALS'/*/WP_I18N_GLOBALS_OVERWRITE*/ );
...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) )
		die( sprintf( /*WP_I18N_OLD_PHP*/'O seu servidor está a correr o PHP na versão %1$s mas o WordPress %2$s necessita de, pelo menos, a versão %3$s.'/*/WP_I18N_OLD_PHP*/, $php_version, $wp_version, $required_php_version ) );

	if ( !extension_loaded( 'mysql' ) && !file_exists( WP_CONTENT_DIR . '/db.php' ) )
		die( /*WP_I18N_OLD_MYSQL*/'A sua instalação de PHP parece não ter a extensão MySQL, requerida pelo WordPress.'/*/WP_I18N_OLD_MYSQL*/ );
}
...
?>
	...
	<body>
		<h1><?php echo /*WP_I18N_MAINT_MSG*/'De momento indisponível para uma breve manutenção agendada. Volte dentro de uns minutos.'/*/WP_I18N_MAINT_MSG*/; ?></h1>
	</body>
	</html>
<?php
	die();
...
function wp_set_wpdb_vars() {
	...
	if ( is_wp_error( $prefix ) )
		wp_die( /*WP_I18N_BAD_PREFIX*/'<strong>ERRO</strong>: O prefixo <code>$table_prefix</code> no ficheiro <code>wp-config.php</code> só pode conter dígitos, letras e underscores.'/*/WP_I18N_BAD_PREFIX*/ );
}
