...
function rolo_create_initial_pages() {
    ...
    $add_contact_page = get_page_by_title("Adicionar Contacto");
    if (!($add_contact_page->ID >  0)) {
        $new_page = array();
        $new_page['post_title'] = 'Adicionar Contacto';
        $new_page['post_type'] = 'page';
        $new_page['post_content'] = 'Esta página foi criada automáticamente pela aplicação.  Não a remova.';
    ...
    /* WE DON'T NEED COMPANIES FOR NOW
    ...
    if (!($edit_contact_page->ID >  0)) {
        $new_page = array();
        $new_page['post_title'] = 'Editar Contacto';
        $new_page['post_type'] = 'page';
        $new_page['post_content'] = 'Esta página foi criada automáticamente pela aplicação.  Não a remova.';
        $new_page['post_status'] = 'publish';
    ...
    /* WE DONT NEED COMPANIES FOR NOW
